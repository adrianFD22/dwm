...
enum { NetSupported, NetWMName, NetWMState, NetWMCheck,
       ...
       NetWMWindowTypeDialog, NetWMDesktop, NetClientList, NetDesktopNames, NetDesktopViewport, NetNumberOfDesktops, NetCurrentDesktop, NetLast }; /* EWMH atoms */
...
struct Monitor {
	...
	int gappx, default_gappx;            /* gaps between windows */
...
focusstack(const Arg *arg)
{
	...
	if (!selmon->sel || (selmon->sel->isfullscreen && lockfullscreen))
		return;
	if (arg->i > 0) {
		for (c = selmon->sel->next; c && (!ISVISIBLE(c) || (c->issticky && !c->isfullscreen)); c = c->next);
		if (!c)
			for (c = selmon->clients; c && (!ISVISIBLE(c) || (c->issticky && !c->isfullscreen)); c = c->next);
	} else {
...
runautostart(void)
{
	 ...
	 */
    // TODO: clean this shit, horrible solution
	xdgdatahome = "/home/adrian/.config";
